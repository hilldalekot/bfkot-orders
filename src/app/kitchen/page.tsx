...
export default function KitchenDashboard() {
  ...
  const sendStartersBeveragesToWhatsApp = (roomNumber: string, roomOrders: Order[]) => {
    if (!roomOrders || roomOrders.length === 0) return;
    ...
    let text = `*Room ${roomNumber} - Starters & Beverages Summary*\n`;
    text += `Time: ${timeFormatted}\n\n`;
    ...
    if (beverageKeys.length > 0) {
      beverageKeys.forEach(k => {
        text += `• ${k}: ${beverageCounts[k]}\n`;
      });
    } else {
      text += `_None_\n`;
    }
                      ...
                      <div key={order.id} className="bg-white rounded-xl shadow-sm border border-[var(--stone-200)] flex flex-col">
                        ...
                        <div className="p-3 bg-[var(--stone-50)] border-t border-[var(--stone-100)] flex items-center justify-between rounded-b-xl">
                          ...
                          <select
                            ...
                            disabled={staffRole === "Kitchen Staff"}
                            className="text-xs bg-white border border-[var(--stone-200)] rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[var(--accent-gold)] disabled:bg-gray-100 disabled:text-gray-500"
                          >
